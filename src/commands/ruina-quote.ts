    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (selectedCategory !== null) {
            const filterCategory =
                DialogueCategory[selectedCategory as keyof typeof DialogueCategory];
            dialogues = dialogues.filter((dialogue) => dialogue.category === filterCategory);
        }
        ...
        if (
            DialogueCategory[selectedCategory as keyof typeof DialogueCategory] ===
            DialogueCategory.Story
        ) {
            const sounds = await getSoundsFromDatabase();
            const dialogueSound = sounds.find((x) =>
                x.fileName.includes(randomDialogue.voiceFile + ".wav"),
            );
