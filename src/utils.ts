import { ColorResolvable, CommandInteraction, Guild, GuildBasedChannel } from "discord.js";
import fs from "fs";
...
import { ALLOWED_CHANNEL_IDS, env, TIPHERETH_CARD_RANGE_RAW_DATA_MAP } from "./constants";
...
export const onCommandInteraction = (interaction: CommandInteraction) => {
    console.log(
        `${interaction.createdAt} - Command ${interaction.commandName} used in channel "${interaction.guild?.channels.cache.get(interaction.channelId)?.name}" (${interaction.channelId}) by user ${interaction.user.username} with arguments`,
        interaction.options,
    );
    if (interaction.guild && !ALLOWED_CHANNEL_IDS.includes(interaction.channelId)) {
    ...
    if (useCount >= env.REQUEST_LIMIT) {
        throw new Error(`Exceeded rate limit of ${env.REQUEST_LIMIT} requests per minute.`);
    }
    ++useCount;
};
...
export const findFileRecur: (targetFileName: string, dir: string) => string | null = (
    targetFileName,
    dir,
) => {
...
export const importDefaults: <Type>(dirPath: string) => Promise<Type[]> = async (dirPath) => {
...
export const getGuildChannelsFromIds: (guild: Guild, ids: string[]) => GuildBasedChannel[] = (
    guild,
    ids,
) => {
...
export const getSyntaxForColor: (color: ColorResolvable) => string = (color) => {
    switch (color) {
        case "Yellow":
            return "fix";
        case "Orange":
...
export const getCardDetailHtml: (cardName: string) => Promise<string> = async (cardName) => {
    ...
    const cardDetailLink = document.querySelector(".card_title")?.querySelector("a")?.href;
    if (cardDetailLink === undefined) {
        throw new Error(`Could not get link to detail page for card ${cardName}`);
    }
...
export const getCardDataTiphereth: (cardName: string) => Promise<Card> = async (cardName) => {
    ...
    let cardImgUrl = document.querySelector('[data-label="Artwork"]')?.querySelector("img")?.src;
    ...
    const cardDesc = document.querySelector('.card_script[data-lang="en"]')?.textContent;

    if (cardDesc === null || cardDesc === undefined) {
        throw new Error(`Card ${closestCardName} has null or undefined description!`);
    }
    ...
    cardDiceElems.forEach((diceElem) => {
        ...
        const diceRoll = diceElem.querySelector(".card_dice_range")?.textContent;
        ...
        if (isNaN(minRoll) || isNaN(maxRoll)) {
            throw new Error(`Could not parse dice rolls ${diceRolls} to numbers`);
        }
    ...
    const cardRange = document.querySelector(".card_range .icon")?.getAttribute("title");
    if (!cardRange) {
        throw new Error(`Unknown card range: ${cardRange}`);
    }

    const cardRarity = document.querySelector(".ent_info_table")?.getAttribute("data-rarity");
    if (!cardRarity) {
...
export const getAbnoPagesFromXml = (xml: string) => {
    ...
    const abnoInfoFile = fs.readFileSync(
        `${env.EXTRACTED_ASSETS_DIR}/text/EN/EN_AbnormalityCards.txt`,
    );
    ...
    try {
        ...
        for (const emotionCard of emotionCards) {
            ...
            if (abnoInfo === undefined) {
                console.error(
                    `Cannot find description for abnormality page ${pageId}`,
                    inspect(abnoPageJs, { depth: null }),
                );
            }
            const dialogue = [];
            ...
            const page: AbnoPage = {
                ...
                targetType:
                    AbnoTargetType[emotionCard["TargetType"] as keyof typeof AbnoTargetType],
...
export const getCardsFromXml = (xml: string) => {
    ...
    const getAbilityText = (cardId: string, abilityId: string, diceIndex?: number) => {
        ...
        for (const abilityFile of abilityFiles) {
            ...
            const abilities: any[] = abilitiesJs["BattleCardAbilityDescRoot"]["BattleCardAbility"];
        ...
        for (const cardFile of cardFiles) {
            ...
            const cardDescs: any[] = cardJs["BattleCardDescRoot"]["cardDescList"]["BattleCardDesc"];
            const cardDesc = cardDescs.find((desc) => desc["@_ID"] === cardId);
            if (cardDesc === undefined) {
                // may be a Korean-only card
                continue;
            ...
            if (diceIndex !== undefined) {
                ...
                if (diceDescs) {
                    abilityDesc = diceDescs.find((desc) => desc["@_ID"] === diceIndex.toString());
                    if (abilityDesc) {
    ...
    for (const jsCard of jsCards) {
        ...
        for (const [i, behaviour] of diceBehaviours.entries()) {
            const category = DiceCategory[behaviour["@_Type"] as keyof typeof DiceCategory];
            const type = DiceType[behaviour["@_Detail"] as keyof typeof DiceType];
        ...
        console.log(
            `get card ${cards.length + 1}/${Object.keys(jsCards).length} ${cardName} from xml`,
        );
        const cardImage = jsCard["Artwork"];
        if (!doesImageExist(cardImage)) {
            // probably a card that is cut from the game
            console.log(`skipping card ${cardName} (${cardId}) because it has no image`);
            continue;
        }
...
export const getTextHeight = (context: Canvas.CanvasRenderingContext2D, text: string) => {
